...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.openobservatory.ooniprobe$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.openobservatory.ooniprobe.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall org.openobservatory.ooniprobe > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall org.openobservatory.ooniprobe.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear org.openobservatory.ooniprobe > /dev/null; \
fi)
...
app-original.apk : 
	...
	$(GW) -PjacocoEnabled=false assembleFdroid </dev/null && \
	mv app/build/outputs/apk/stableFdroid/debug/app-stable-fdroid-debug.apk app-debug.apk && \
...
app-androidTest.apk :
	...
	mv app/build/outputs/apk/androidTest/stableFdroid/debug/app-stable-fdroid-debug-androidTest.apk app-androidTest-unsigned.apk && \
...
app-original-for-coverage.apk app-androidTest-for-coverage.apk:
	@export ABC_CONFIG=$(ABC_CFG) && \
	$(GW) -PjacocoEnabled=true clean assembleFdroid assembleAndroidTest </dev/null && \
	mv app/build/outputs/apk/stableFdroid/debug/app-stable-fdroid-debug.apk app-original-for-coverage.apk  && \
	mv app/build/outputs/apk/androidTest/stableFdroid/debug/app-stable-fdroid-debug-androidTest.apk app-androidTest-for-coverage.apk
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) org.openobservatory.ooniprobe.test/org.openobservatory.ooniprobe.TestAndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces org.openobservatory.ooniprobe ./traces/$(TEST_NAME) force-clean
