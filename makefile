...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=org.openobservatory.ooniprobe"
...
app-original-with-jacoco.apk :
	export ABC_CONFIG=$(ABC_CFG) && \
	$(GW) -PjacocoEnabled=true assembleFdroid && \
	mv app/build/outputs/apk/stableFdroid/debug/app-stable-fdroid-debug.apk app-debug.apk && \
	$(ABC) sign-apk app-debug.apk && \
	mv -v app-debug.apk app-original.apk
...
espresso-tests.log : app-original.apk app-androidTest.apk running-emulator
	...
	$(ADB) shell am instrument -w -r org.openobservatory.ooniprobe.test/org.openobservatory.ooniprobe.TestAndroidJUnitRunner | tee espresso-tests.log
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) stop-all-emulators
	$(RM) running-emulator
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c org.openobservatory.ooniprobe))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear org.openobservatory.ooniprobe; \
	else \
	...
	$(ADB) shell am instrument -w -e class $(TEST_NAME) org.openobservatory.ooniprobe.test/org.openobservatory.ooniprobe.TestAndroidJUnitRunner | tee $(@)
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/org/openobservatory/ooniprobe
